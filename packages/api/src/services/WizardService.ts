  ...
  public async upsertGuestSessionAndSaveWizardData(
    ...
    if (data.workExperiences) {
      for (const we of data.workExperiences) {
        if (we.id) {
          await this.db.workExperience.updateMany({ where: { id: we.id, cvId: cv.id }, data: { jobTitle: we.jobTitle, company: we.company, startDate: we.startDate, endDate: we.endDate, description: we.description } });
        } else {
    ...
    if (data.educations) {
      for (const ed of data.educations) {
        if (ed.id) {
          await this.db.education.updateMany({ where: { id: ed.id, cvId: cv.id }, data: { degree: ed.degree, institution: ed.institution, startDate: ed.startDate, endDate: ed.endDate, description: ed.description } });
        } else {
    ...
    if (data.skills) {
      for (const sk of data.skills) {
        if (sk.id) {
          await this.db.skill.updateMany({ where: { id: sk.id, cvId: cv.id }, data: { name: sk.name, level: sk.level } });
        } else {
